...
df <- bind_rows(dfold, X03_22_2020, X03_23_2020, X03_24_2020, X03_25_2020, X03_26_2020)
df$date <- mdy(df$date)
...
```{r states}
states <- df %>% filter(country_region == "US") %>% filter(province_state != "Diamond Princess" | province_state != "Grand Princess" | province_state != "Wuhan Evacuee" | province_state != "Recovered")
...
states <- states %>% group_by(date, province_state,lat, long) %>% summarize(confirmed = sum(confirmed), deaths = sum(deaths), recovered = sum(recovered))
